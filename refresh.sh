...
do
  ...
  diff -a -y --suppress-common-lines <(echo -e "\n${file_name}: *NIIS*"; cat ${DIR}/${file_name}.md) <(echo  "*EE*"; cat ${DIR}/${file_name}.md) | sed 2i======================== >> diff_$1.md
  x=$(( $x + 1 ))
done < "$INFILE"
...
do
    ...
    then
        ...
        echo -e "${YELLOW} ${file_name}.md changed ${YELLOW}"
        diff -a -y --suppress-common-lines <(echo -e "\n${file_name}: *NIIS*"; cat ${DIR}/${file_name}.md) <(echo "*EE*"; cat md/${file_name}.md) | sed 2i===================== >> diff_$1.md
    fi
done
